...
const Courses1 = () => {
  ...
  const [searchQueryUseFor, setSearchQueryUseFor] = useState("");
  const [filteredCourses, setFilteredCourses] = useState([]);
  ...
  const handleSearch = (e) => {
    ...
    const queryUseFor = searchQueryUseFor.toLowerCase();
    let filtered = courses;

    if (queryName.trim() !== "") {
      filtered = filtered.filter((course) =>
        course.coursename.toLowerCase().includes(queryName)
      );
    }

    if (queryUseFor.trim() !== "") {
      filtered = filtered.filter((course) =>
        course.courseusefor.toLowerCase().includes(queryUseFor)
      );
    }
      ...
      <div className='bg-gradient-to-r from-sky-900 via-teal-700 to-sky-900 w-screen font-medium absolute inset-x-0 top-0 overflow-x-hidden'>
        ...
        <form onSubmit={handleSearch}>
          <div className="relative mx-auto max-w-md mt-4">
            <label htmlFor="search_name" className="text-xs font-medium text-gray-900 sr-only dark:text-white">Search by Course Name</label>
            <input type="search" id="search_name" className="block w-full p-2 ps-7 text-xs text-violet-900 border border-gray-300 rounded-lg bg-cyan-900 focus:ring-blue-500 focus:border-blue-500 dark:bg-cyan-700 dark:border-cyan-600 dark:placeholder-cyan-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search by Course Name..." value={searchQueryName} onChange={(e) => setSearchQueryName(e.target.value)}  />
          </div>
          <div className="relative mx-auto max-w-md mt-4">
            <label htmlFor="search_use_for" className="text-xs font-medium text-gray-900 sr-only dark:text-white">Search by Course Use For</label>
            <input type="search" id="search_use_for" className="block w-full p-2 ps-7 text-xs text-violet-900 border border-gray-300 rounded-lg bg-cyan-900 focus:ring-blue-500 focus:border-blue-500 dark:bg-cyan-700 dark:border-cyan-600 dark:placeholder-cyan-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search by Course Use For..." value={searchQueryUseFor} onChange={(e) => setSearchQueryUseFor(e.target.value)}  />
          </div>
          <button type="submit" className="text-white bg-green-950 absolute end-1 bottom-1 focus:ring-2 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-1 dark:bg-green-900 dark:hover:bg-green-950 dark:focus:ring-green-800">Search</button>
        </form>
